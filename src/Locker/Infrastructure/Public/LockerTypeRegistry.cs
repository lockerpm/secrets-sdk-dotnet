namespace Locker
{
    ...
    public static class LockerTypeRegistry
    {
        ...
        {
            ArgumentNullException.ThrowIfNull(potentialType);
            if (!potentialType.GetTypeInfo().IsInterface)
            {
                return potentialType;
            }

            if (!string.IsNullOrEmpty(objectValue)
                && ObjectToTypes.TryGetValue(objectValue, out var concreteType)
                && concreteType is not null)
            {
